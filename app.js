...
function monthGenitive(monthIndex) {
  ...
  if (nominative.endsWith("ь") || nominative.endsWith("й"))
    return nominative.slice(0, -1) + "я";
  return nominative + "а";
}
...
function normalizeTime(v) {
  ...
  if (typeof v === "string") {
    ...
    if (s !== "" && !Number.isNaN(n) && /^-?[\d.,]+$/.test(s))
      return normalizeTime(n);
  }
...
function rowToLesson(row, sheetRowIndex) {
  ...
  const hasObjectData =
    !isArrayRow && typeof row === "object" && Object.keys(row).length > 0;
  if (!hasArrayData && !hasObjectData) return null;

  const idRaw = isArrayRow ? "" : getObjectField(row, ["id", "ID", "Id"]);
  const dayRaw = isArrayRow
    ? row[0]
    : getObjectField(row, ["day", "день", "День"]);
  const pairRaw = isArrayRow
    ? row[1]
    : getObjectField(row, [
        "pair",
        "pairNum",
  ...
  const startRaw = isArrayRow
    ? row[2]
    : getObjectField(row, ["start", "startTime", "начало", "Начало"]);
  const endRaw = isArrayRow
    ? row[3]
    : getObjectField(row, ["end", "endTime", "конец", "Конец", "Окончание"]);
  const typeRaw = isArrayRow
    ? row[4]
    : getObjectField(row, ["type", "тип", "Тип"]);
  const subjectRaw = isArrayRow
    ? row[5]
    : getObjectField(row, ["subject", "дисциплина", "предмет", "Дисциплина"]);
  const roomRaw = isArrayRow
    ? row[6]
    : getObjectField(row, ["room", "аудитория", "кабинет", "Аудитория"]);
  const roomSchemeUrlRaw = isArrayRow
    ? row[7]
    : getObjectField(row, [
        "roomSchemeUrl",
        "roomPhotoUrl",
  ...
  const teacherRaw = isArrayRow
    ? row[8]
    : getObjectField(row, ["teacher", "преподаватель", "Преподаватель"]);
  const weekRaw = isArrayRow
    ? row[9]
    : getObjectField(row, ["week", "неделя", "Неделя"]);
...
function parseSessionItem(row, sheetRowIndex) {
  ...
  const subject = String(
    getObjectField(row, ["Дисциплина", "subject", "дисциплина"]) ?? "",
  ).trim();
  const room = String(
    getObjectField(row, ["Аудитория", "room", "аудитория"]) ?? "",
  ).trim();
  const roomSchemeUrl = String(
    getObjectField(row, ["Схема аудитории", "roomSchemeUrl", "схема"]) ?? "",
  ).trim();
  const teacher = String(
    getObjectField(row, ["Преподаватель", "teacher"]) ?? "",
  ).trim();
...
function parseLinkItem(row, sheetRowIndex) {
  ...
  const url = String(
    getObjectField(row, ["url", "URL", "ссылка", "Ссылка", "href", "link"]) ??
      "",
  ).trim();
...
function extractRowsFromJson(j) {
  ...
  if (j.data && j.data.values && Array.isArray(j.data.values))
...
createApp({
  setup() {
    ...
    const accentPalette = Object.entries(accentColors).map(([key, meta]) => ({
      key,
      ...meta,
    }));
    const lessonSchemeChoices = Object.entries(lessonColorSchemes).map(
      ([key, schemeData]) => ({ key, schemeData }),
    );
    const glassBgChoices = Object.entries(glassBackgrounds).map(
      ([key, bgData]) => ({ key, bgData }),
    );
    ...
    function scrollActiveFilterTabIntoView(behavior) {
      ...
      nextTick(() => {
        ...
        if (!target && activeSheet.value === "schedule") {
          target =
            container.querySelector(".schedule-fil-wrap .filter-pill") ||
    ...
    function onFilterBarPointerDown(e) {
      ...
      if (!onThumb && maxThumbTravel > 0) {
        const targetLeft = Math.max(
          0,
          Math.min(maxThumbTravel, clickX - thumbW / 2),
        );
      ...
      const onMove = (ev) => {
        ...
        if (filterBarDrag.maxThumbTravel > 0) {
          el.scrollLeft =
            filterBarDrag.startScroll +
            (dx / filterBarDrag.maxThumbTravel) * filterBarDrag.maxScroll;
        }
        updateFilterBarThumb();
      };
      const onUp = () => {
    ...
    function wLbl(w) {
      return { both: "Обе", odd: "Нечётная", even: "Чётная" }[w] || w;
    }
    function pN(lesson) {
    ...
    function vucRemainderForDate(date) {
      ...
      const anchorMon = mondayOfCalendarWeek(
        new Date(VUC_ANCHOR_DT.y, VUC_ANCHOR_DT.m, VUC_ANCHOR_DT.d),
      );
      ...
      const remaining =
        weekDelta < 0
          ? VUC_REMAIN_AT_ANCHOR_WEEK
          : Math.max(0, VUC_REMAIN_AT_ANCHOR_WEEK - weekDelta);
    ...
    function buildDays(src) {
      ...
      for (let i = 0; i < 14; i++) {
        ...
        if (meta) {
          ...
          else if (meta.cls === "holiday")
            weekendLabel = meta.shortLabel || "Праздник";
          else if (meta.shortLabel) weekendLabel = meta.shortLabel;
        }

        let lessons = [];
        if (!isWeekend) {
          const allLessons = filterVUC(src).filter(
            (l) => l.day === dayName && wm(l, dayWt),
          );
          lessons = sortL(allLessons);
          if (meta && meta.preHoliday)
            lessons = lessons.filter(
              (l) => timeToMin(l.start) <= timeToMin("14:30"),
            );
        }
    ...
    const sessionDays = computed(() => {
      ...
      for (const sess of sessions.value) {
        const key = sess.dateSort
          ? String(sess.dateSort)
          : sess.dateLabel || "id-" + sess.id;
        if (!byKey.has(key)) {
          byKey.set(key, {
    ...
    const scheduleVisList = computed(() =>
      sch.value.filter((l) => l.subject !== "ВУЦ"),
    );
    ...
    function prevM() {
      calDir.value = "prev";
      const m = calM.value;
      calM.value = new Date(m.getFullYear(), m.getMonth() - 1, 1);
    }
    function nextM() {
      calDir.value = "next";
      const m = calM.value;
    ...
    function isHoliday(d) {
      const m = d.getMonth(),
        dd = d.getDate();
      return FIXED_HOLIDAYS.some(([mm, ddd]) => mm === m && ddd === dd);
    }
    function isPreHoliday(d) {
      const m = d.getMonth(),
        dd = d.getDate();
      return PRE_HOLIDAYS.some(([mm, ddd]) => mm === m && ddd === dd);
    }
    function isPreMayCutoff(d) {
      const m = d.getMonth(),
        dd = d.getDate();
      return PRE_MAY_CUTOFF.some(([mm, ddd]) => mm === m && ddd === dd);
    }
    function getHolidayLabel(d) {
      return FIXED_HOLIDAY_LABELS[d.getMonth() + "," + d.getDate()] || "";
    }
    function getPeriodAfterTeaching(d) {
      const m = d.getMonth(),
        dd = d.getDate();
      if (m === 5 && dd >= 1 && dd <= 7)
        return { short: "Зач.нед", cls: "credit-week" };
      if (m === 5 && dd >= 8) return { short: "Сессия", cls: "session" };
      if (m === 6 && dd <= 5) return { short: "Сессия", cls: "session" };
      if (m === 6 && dd >= 6 && dd <= 19)
        return { short: "Практика", cls: "practice" };
    ...
    function getCellMeta(d) {
      const dow = d.getDay(),
        period = getPeriodAfterTeaching(d),
        holiday = isHoliday(d);
      if (period)
        return {
          cls: period.cls,
      ...
      if (holiday) {
        const lbl = getHolidayLabel(d);
        return {
          cls: "holiday",
          shortLabel: lbl || "Праздник",
          dots: [],
          preHoliday: false,
        };
      }
      if (dow === 0)
        return { cls: "weekend", shortLabel: "", dots: [], preHoliday: false };
      if (dow === 6 && isSaturdayWeekend(d))
        return { cls: "weekend", shortLabel: "", dots: [], preHoliday: false };
      if (isPreHoliday(d))
        return {
          cls: "pre-holiday",
    ...
    const calCells = computed(() => {
      const m = calM.value,
        f = new Date(m.getFullYear(), m.getMonth(), 1),
      ...
      for (let d = 1; d <= l.getDate(); d++) {
        ...
        const hasSess = sessionsByDate.value.has(dk);
        if (
          meta &&
          [
            "weekend",
          ...
          cs.push({
            day: d,
            date,
            cls: hasSess ? "exam-day" : meta.cls,
            shortLabel: hasSess ? "" : meta.shortLabel || "",
            dots: hasSess ? ["exam"] : [],
          });
          continue;
        }
        ...
        let ls = filterVUC(
          sch.value.filter((x) => x.day === DAYS[idx] && wm(x, wt(date))),
          DAYS[idx],
        );
        if (meta && meta.preHoliday)
          ls = ls.filter((x) => timeToMin(x.start) <= timeToMin("14:30"));
        ...
        if (hasSess) dotTypes.push("exam");
        cs.push({
          day: d,
          date,
          cls: hasSess ? "exam-day" : (meta ? meta.cls : ""),
          shortLabel: meta ? meta.shortLabel || "" : "",
          dots: dotTypes,
        });
      }
    ...
    function fmtD(d) {
      ...
      return `${DAYS[i]}, ${d.getDate()} ${monthGenitive(d.getMonth())}`;
    }
    function dateKey(d) {
    ...
    const selL = computed(() => {
      ...
      const date = selD.value,
        meta = getCellMeta(date);
      if (
        meta &&
        [
          "weekend",
      ...
      let ls = filterVUC(
        sch.value.filter((l) => l.day === DAYS[i] && wm(l, wt(date))),
        DAYS[i],
      );
      if (meta && meta.preHoliday)
        ls = ls.filter((l) => timeToMin(l.start) <= timeToMin("14:30"));
      ls = ls.filter((l) => lessonShownLesson(l));
      return sortL(ls);
    });

    const selSessionItems = computed(() => {
      if (!selD.value) return [];
      return sessionsByDate.value.get(dateKey(selD.value)) || [];
    });

    const selPeriod = computed(() => {
      if (!selD.value) return "";
      if (isHoliday(selD.value))
        return getHolidayLabel(selD.value) || "Праздник";
    ...
    async function loadSessionDataInBackground() {
      const cfg =
        typeof window.SCHEDULE_CONFIG === "object" && window.SCHEDULE_CONFIG
          ? window.SCHEDULE_CONFIG
          : {};
      ...
      try {
        const rows = await fetchSheetFromConfig(cfg, "session", {});
        const items = parseSessionsData(rows);
        sessions.value = items;
        const now = new Date().toISOString();
        sessionsFetchedAt = now;
    ...
    return {
      ...
      loadingLabel,
      showScheduleFilMenu,
      schedulePillPressing,
      onSchedulePillPointerDown,
      onSchedulePillPointerUp,
      onSchedulePillClick,
      openScheduleFilMenu,
      selectScheduleFil,
      sheetSwitchAnim,
